...
export default function BadgeUpdateSheet({ onSave }: { onSave?: (badgeUrl: string) => void }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Toast.show({ type: "success", text1: "Badge Updated!", text2: "Your badge is now live." })
    } catch {
      Toast.show({ type: "error", text1: "Error", text2: "Could not update badge." })
    } finally {
      setLoading(false)
    }
  }
  ...
  const renderBadge = (url: string, index: number, locked: boolean = false) => {
    ...
    return (
      ...
      >
        <Image source={{ uri: url }} style={styles.badgeImage} resizeMode="contain" />
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.subtitle}>Your badge appears next to your name in all chats</Text>
      ...
      <View style={styles.preview}>
        {selected ? (
          ...
          <View style={styles.previewPlaceholder}>
            <Text style={styles.previewPlaceholderText}>No badge selected</Text>
          </View>
        )}
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.grid}>
          {badgesFree.map((url: string, i: number) => renderBadge(url, i, false))}
        </View>

        {/* VIP Badges */}
        <Text style={styles.sectionTitle}>👑 VIP Badges</Text>
        <View style={styles.grid}>
          {badgesVip.map((url: string, i: number) => renderBadge(url, i, !isVipUser))}
        </View>
      ...
      {/* Save Button */}
      <TouchableOpacity
        style={[styles.saveBtn, (selected === user?.badge || loading) && styles.saveBtnDisabled]}
        onPress={handleSave}
...
const styles = StyleSheet.create({
  ...
  // ── Save ─────────────────────────────────────────────
  saveBtn: {
    height: 52, backgroundColor: "#D32F2F", borderRadius: 50,
    alignItems: "center", justifyContent: "center", marginTop: 4,
    shadowColor: "#D32F2F", shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.3, shadowRadius: 10, elevation: 5,
  },
  saveBtnDisabled: { backgroundColor: "#e08080", shadowOpacity: 0, elevation: 0 },
  saveBtnText: { color: "#fff", fontSize: 16, fontWeight: "700", letterSpacing: 0.4 },
})